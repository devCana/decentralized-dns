...
describe("NamespaceDApp", () => {
  ...
  describe("pricing", () => {
    ...
    it("rejects invalid lengths", async () => {
      ...
      await expect(dapp.priceOf("a".repeat(64))).to.be.revertedWithCustomError(
        dapp,
        "InvalidName"
      );
    });
  ...
  describe("register (UC-1)", () => {
    ...
    it("rejects an insufficient fee", async () => {
      const { dapp, alice } = await loadFixture(deployFixture);
      await expect(
        dapp.connect(alice).register("ab", PUBKEY, { value: BASE_PRICE })
      ).to.be.revertedWithCustomError(dapp, "InsufficientFee");
    });
    ...
    it("refunds overpayment", async () => {
      ...
      const tx = await dapp
        .connect(alice)
        .register("example", PUBKEY, { value: BASE_PRICE * 3n });
    ...
    it("rejects a taken name", async () => {
      ...
      await expect(
        dapp.connect(bob).register("example", PUBKEY2, { value: BASE_PRICE })
      ).to.be.revertedWithCustomError(dapp, "NameUnavailable");
    });
    ...
    it("rejects an empty or oversized pubKey", async () => {
      const { dapp, alice } = await loadFixture(deployFixture);
      await expect(
        dapp.connect(alice).register("example", "0x", { value: BASE_PRICE })
      ).to.be.revertedWithCustomError(dapp, "InvalidPubKey");
      await expect(
        dapp
          .connect(alice)
          .register("example", "0x" + "aa".repeat(129), { value: BASE_PRICE })
      ).to.be.revertedWithCustomError(dapp, "InvalidPubKey");
    });
  });
  ...
  describe("renew", () => {
    it("extends expiry by one period", async () => {
      ...
      await expect(
        dapp.connect(bob).renew("example", { value: BASE_PRICE })
      ).to.emit(dapp, "Renewed");
    ...
    it("rejects renewal of unregistered or expired domains", async () => {
      const { dapp, alice } = await loadFixture(deployFixture);
      await expect(
        dapp.connect(alice).renew("ghost", { value: BASE_PRICE })
      ).to.be.revertedWithCustomError(dapp, "DomainNotRegistered");

      await dapp.connect(alice).register("example", PUBKEY, { value: BASE_PRICE });
      await time.increase(YEAR + 1n);
      await expect(
        dapp.connect(alice).renew("example", { value: BASE_PRICE })
    ...
    it("rejects an insufficient renewal fee", async () => {
      ...
      await dapp.connect(alice).register("example", PUBKEY, { value: BASE_PRICE });
      await expect(
        dapp.connect(alice).renew("example", { value: BASE_PRICE - 1n })
      ).to.be.revertedWithCustomError(dapp, "InsufficientFee");
    });
  });
  ...
  describe("transfer (UC-3)", () => {
    ...
    it("rejects transfer by non-owner, to zero, or when expired", async () => {
      ...
      await expect(
        dapp.connect(bob).transfer("example", bob.address, PUBKEY2)
      ).to.be.revertedWithCustomError(dapp, "NotDomainOwner");
      await expect(
        dapp.connect(alice).transfer("example", ethers.ZeroAddress, PUBKEY2)
      ).to.be.revertedWithCustomError(dapp, "ZeroAddress");

      await time.increase(YEAR + 1n);
      await expect(
        dapp.connect(alice).transfer("example", bob.address, PUBKEY2)
      ).to.be.revertedWithCustomError(dapp, "DomainExpired");
    });
  });
  ...
  describe("withdraw", () => {
    it("lets only the treasurer collect fees", async () => {
      ...
      await expect(
        dapp.connect(alice).withdraw(alice.address)
      ).to.be.revertedWithCustomError(dapp, "NotTreasurer");
